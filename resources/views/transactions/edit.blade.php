                        ...
                        <div class="section-container mb-5">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="invoice_number" class="form-control bg-light border-0 font-weight-bold" value="{{ $transaction->invoice_number }}" readonly>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label class="form-label text-muted small text-uppercase font-weight-bold">Pilih Asuransi</label>
                                    <select name="insurance_id" id="insurance_id" class="form-control form-select custom-select border-primary-light" required>
                                        <option value="">-- Pilih Asuransi --</option>
                                        @foreach($insurances as $insurance)
                                            <option value="{{ $insurance['id'] }}" {{ $transaction->insurance_id == $insurance['id'] ? 'selected' : '' }}>{{ $insurance['name'] }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6 mb-3 d-none">
                                    ...
                                    <div id="voucher-status">
                                        ...
                                        <input type="hidden" name="voucher_id" id="voucher_id" value="{{ $transaction->voucher_id }}">
                                    </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="mt-5 text-end">
                            <button type="submit" class="btn btn-primary px-5 shadow-sm font-weight-bold" id="btnSubmit">
                                Perbarui Transaksi <i class="fas fa-save ml-2"></i>
                            </button>
                        </div>
                    </div>
                </div>
...
<template id="rowTemplate">
    <tr class="detail-row">
        <td>
            ...
            <input type="hidden" name="procedure_name"> 
            <input type="hidden" name="price_id"> 
            <div class="small text-muted mt-1 price-dates"></div>
        </td>
        <td>
            <input type="text" name="price_text" class="form-control border-0 bg-transparent font-weight-bold" readonly value="0"> 
            <input type="hidden" name="price" id="price_val" class="form-control border-0 bg-transparent font-weight-bold" readonly value="0"> 
            <input type="hidden" name="price_start_date"> 
            <input type="hidden" name="price_end_date"> 
        </td>
        <td>
        ...
        <td>
            <input type="text" name="row_subtotal_text" class="form-control border-0 bg-transparent font-weight-bold" readonly value="0"> 
            <input type="hidden" name="row_subtotal" class="form-control border-0 bg-transparent font-weight-bold" readonly value="0"> 
        </td>
...
<script id="initial_data" type="application/json">
{!! json_encode([
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const initialDataStr = document.getElementById('initial_data').textContent;
    ...
    function addRow(detail = null) {
        ...
        procSelect.addEventListener('change', function() {
            ...
            // Don't fetch if initializing from details
            if (skipFetch && detail && detail.procedure_id == procId) {
                return;
            }
        ...
        // Populate detail if present
        if (detail) {
            row.querySelector('.procedure-select').value = detail.procedure_id;
            row.querySelector('[name="procedure_name"]').value = detail.procedure_name;
            row.querySelector('[name="price_text"]').value = formatIDR(detail.price);
            row.querySelector('[name="price"]').value = detail.price;
            row.querySelector('[name="price_id"]').value = detail.price_id;
            row.querySelector('[name="price_start_date"]').value = detail.price_start_date;
            row.querySelector('[name="price_end_date"]').value = detail.price_end_date;
            row.querySelector('.price-dates').textContent = `Berlaku: ${detail.price_start_date} s/d ${detail.price_end_date}`;
            row.querySelector('[name="qty"]').value = detail.qty;
            row.querySelector('[name="discount_per_item"]').value = detail.discount_per_item;
            row.querySelector('[name="discount_per_item_text"]').value = formatIDR(detail.discount_per_item);
    ...
    transactionForm.addEventListener('submit', function(e) {
        ...
        fetch(`{{ route('transactions.update', $transaction->id) }}`, {
            method: 'PUT',
            headers: {
        ...
        .catch(err => {
            ...
            btnSubmit.innerHTML = 'Perbarui Transaksi <i class="fas fa-save ml-2"></i>';
        });
    });
